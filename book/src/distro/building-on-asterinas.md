...
- a writable Nix store, so Nix can realize the development-shell closure;
...
The dev shell exports `LINUX_EFI_GRUB_MKRESCUE` (the kernel build otherwise
expects the Docker image's grub-mkrescue path) and pins `NIX_PATH` to the
flake-locked nixpkgs (the ISO expression evaluates `<nixpkgs>`).
...
rescue ISO and the Nix-provided OVMF firmware, without network, display, or
test disks. `AUTO_TEST=boot` fails unless `qemu.log` contains
`Successfully booted.`
