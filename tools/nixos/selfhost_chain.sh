...
SERVE_PID=$!
# connect-timeout 0: with one connection every NAR request waits in curl's queue,
# and Nix 2.34's 15 s default counts that wait as a connect failure.
SUBST_OPTS='--option http-connections 1 --option connect-timeout 0 --option extra-substituters http://10.0.2.2:18081'
...
# tools/qemu_args.sh draws host-forward ports from 1024-65535; one already used
# by an outgoing connection (ephemeral range 32768-60999) makes QEMU refuse to
# start. The chain never uses the forwards, so pin them below that range.
...
send 'printf "CHECKED_HEAD="; git rev-parse HEAD'
wait_for 'CHECKED_HEAD=e0f972c7de70f2345773d87715bb18cc18643cef' 60 "the exact rebased revision"
